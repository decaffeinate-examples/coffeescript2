...
const {starts, ends, repeat, compact, count, merge, extend, flatten, del, baseFileName} = CoffeeScript.helpers;
...
test("the `count` helper counts the number of occurrences of a string in another string", function() {
  eq(1/0, count('abc', ''));
  eq(0, count('abc', 'z'));
  eq(1, count('abc', 'a'));
  eq(1, count('abc', 'b'));
...
test("the `merge` helper makes a new object with all properties of the objects given as its arguments", function() {
  ...
  return (() => {
    const result = [];
    for (let key of Object.keys(ary || {})) {
      const val = ary[key];
