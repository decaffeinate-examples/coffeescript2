...
test("async return value (implicit)", function() {
  let out = null;
  const a = async function() {
    const x = await winning(5);
    const y = await winning(4);
...
test("async return value (explicit)", function() {
  let out = null;
  const a = async () => [5, 2, 3];
...
test("async parameters", function() {
...
test("async `this` scoping", function() {
  let bnd = null;
  let ubnd = null;
  let nst = null;
  const obj = {
...
test("await precedence", function() {
  let out = null;
...
test("`await` inside IIFEs", function() {
  let [x, y, z] = new Array(3);

  var a = (async function() {
    x = await (async () => { switch (4) {  // switch 4
      case 2:
...
test("implicit call with `await`", async function() {
...
test("async methods in classes", async function() {
  class Base {
    static async static() {
      return await 1;
    }
