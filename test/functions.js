...
test('#4005: `([a = {}]..., b) ->` weirdness', () => {
  let fn = function (...args) {
    const adjustedLength = Math.max(args.length, 1); const [a = {}] = args.slice(0, adjustedLength - 1); const
...
test('default values with splatted arguments', () => {
  let withSplats = function (a, ...rest) {
    if (a == null) { a = 2; } const adjustedLength = Math.max(rest.length, 2); const b = rest.slice(0, adjustedLength - 2); const val = rest[adjustedLength - 2]; const c = val !== undefined ? val : 3; const val1 = rest[adjustedLength - 1]; const
      d = val1 !== undefined ? val1 : 5; return a * (b.length + 1) * c * d;
  };
  eq(30, withSplats());
  eq(15, withSplats(1));
  eq(5, withSplats(1, 1));
...
test('#4673: complex destructured object spread variables', () => {
  const f = function (...args) {
    const obj1 = args[0]; const
      { ...a } = __objectWithoutKeys__(obj1, []);
    return a;
  };
  eq(f({ c: 1 }).c, 1);
...
test('#4657: destructured array parameters', () => {
  const f = function (...args) {
    const array = args[0]; const adjustedLength = Math.max(array.length, 1); const a = array.slice(0, adjustedLength - 1); const
      b = array[adjustedLength - 1]; return { a, b };
  };
