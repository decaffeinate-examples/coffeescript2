...
test("multiple generated references", function() {
  ...
  let c = 0;
  const d = [];
  return ok(a.b[0<(middle = ++c) && middle<2](...d));
});
...
test("transpile option, for Node API CoffeeScript.compile", function() {
  if (global.testingBrowser) { return; }
  return ok(transpile('compile', "import fs from 'fs'").includes('require'));
});

test("transpile option, for Node API CoffeeScript.eval", function() {
  if (global.testingBrowser) { return; }
  return ok(transpile('eval', "import path from 'path'; path.sep in ['/', '\\\\']"));
});

test("transpile option, for Node API CoffeeScript.run", function() {
  if (global.testingBrowser) { return; }
  return doesNotThrow(() => transpile('run', "import fs from 'fs'"));
});

test("transpile option has merged source maps", function() {
  if (global.testingBrowser) { return; }
...
test("using transpile from the Node API requires an object", function() {
  try {
...
test("transpile option applies to imported .coffee files", function() {
  if (global.testingBrowser) { return; }
