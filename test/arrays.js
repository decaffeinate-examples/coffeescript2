...
test("array elisions indentation and commas", function() {
  const arr1 = [
    , 1, 2, , , 3,
    4, 5, 6
    , , 8, 9,
...
test("array elisions destructuring", function() {
  let a, b, c, d, e, f, g, h;
...
test("array elisions destructuring with splats and expansions", function() {
  let a, b;
  const arr = [1,2,3,4,5,6,7,8,9];
  [,a,,,...b] = arr;
  arrayEq([a,b], [2,[5,6,7,8,9]]);
  const c = arr[1], d = arr[arr.length - 3], e = arr[arr.length - 1];
  arrayEq([c,d,e], [2,7,9]);
  const f = arr[arr.length - 6], g = arr[arr.length - 3];
  return arrayEq([f,g], [4,7]);
});

test("array elisions as function parameters", function() {
  let b, c, d;
  const arr = [1,2,3,4,5,6,7,8,9];
  let foo = ([,a]) => a;
  let a = foo(arr);
...
test("array elisions nested destructuring", function() {
  let a, b, c, d, w;
  const arr = [
    1,
    [2,3, [4,5,6, [7,8,9] ] ]
...
test("#4260: splat after existential operator soak", function() {
...
test("#1349: trailing if after splat", function() {
  let left, left1, left2, left3;
  const a = [3];
  const b = true;
  const c = null;
...
test("splat extraction from generators", function() {
  const gen = function*() {
    yield 1;
    yield 2;
    return yield 3;
