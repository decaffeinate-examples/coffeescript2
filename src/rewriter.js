...
const moveComments = function (fromToken, toToken) {
  ...
  if (toToken.comments && (toToken.comments.length !== 0)) {
    ...
    for (const comment of Array.from(fromToken.comments)) {
      if (comment.unshift) {
  ...
  Rewriter = class Rewriter {
    ...
    rewrite(tokens) {
      ...
      if (__guard__(typeof process !== 'undefined' && process !== null ? process.env : undefined, (x) => x.DEBUG_TOKEN_STREAM)) {
        ...
        console.log(((() => {
          const result = [];
          for (t of Array.from(this.tokens)) {
            result.push(`${t[0]}/${t[1]}${t.comments ? '*' : ''}`);
          }
          return result;
        })()).join(' '));
      }
      ...
      if (__guard__(typeof process !== 'undefined' && process !== null ? process.env : undefined, (x1) => x1.DEBUG_REWRITTEN_TOKEN_STREAM)) {
        ...
        console.log(((() => {
          const result1 = [];
          for (t of Array.from(this.tokens)) {
            result1.push(`${t[0]}/${t[1]}${t.comments ? '*' : ''}`);
          }
          return result1;
        })()).join(' '));
      }
    ...
    removeLeadingNewlines() {
      ...
      for (const leadingNewlineToken of Array.from(this.tokens.slice(0, i))) {
    ...
    addImplicitBracesAndParens() {
      ...
      return this.scanTokens(function (token, i, tokens) {
        let needle; let needle3; let nextToken; let prevToken; let stackIdx; let stackTag; let
          startsLine;
        let [tag] = token;
...
for (const [left, right] of Array.from(BALANCED_PAIRS)) {
