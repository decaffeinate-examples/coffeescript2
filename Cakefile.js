...
`;
  for (let name of ['helpers', 'rewriter', 'lexer', 'parser', 'scope', 'nodes', 'sourcemap', 'coffeescript', 'browser']) {
    code += `\
...
`;
  ...
  if (watch) {
    for (let target of [indexFile, siteSourceFolder, examplesSourceFolder, sectionsSourceFolder]) {
...
const buildAnnotatedSource = function(watch) {
  ...
  (generateAnnotatedSource = function() {
    exec(`cd src && ../node_modules/docco/bin/docco *.*coffee --output ../docs/v${majorVersion}/annotated-source`, function(err) { if (err) { throw err; } });
...
task('test:integrations', 'test the module integrated with other libraries and environments', function() {
  ...
  return webpack({
    ...
    if (err || stats.hasErrors()) {
      ...
      if (stats.hasErrors()) {
        for (let error of Array.from(stats.compilation.errors)) { console.error(error); }
      }
      if (stats.hasWarnings()) {
        for (let warning of Array.from(stats.compilation.warnings)) { console.warn(warning); }
      }
      process.exit(1);
    }
